...
run_everything <- function()
{
  ...
  #1. label_generatation is something you do most days: massage the raw packet captures and send it through the saved model, and get the predicted start and end times of events.
  ...
  #3. training_and_testing is the full package where you (re)train the model. It assumes you have a training dataset with start and end times of events, 
  ...
  run_mode <- readline("What do you want to do? 1) Label generation, 2) Retraining only, 3) Full training and testing: ")
